...
void auto_init_configuration_import(void)
{
    auto_init_configuration_root_backend_init();
    CONF_KEY(root_key, 0, CONFIGURATION_KEY_BUF_MAX);
    configuration_lock(&root_key);
    configuration_import(&root_key);
    configuration_unlock(&root_key);
}

AUTO_INIT_CONFIGURATION(auto_init_configuration_import, CONFIG_AUTO_INIT_CONFIGURATION_IMPORT_PRIO);
