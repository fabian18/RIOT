                                       ...
                                       const _rtr_adv_params_t *params)
{
    ...
    if (netif->ipv6.send_ctx.ra_nonce_size) {
        gnrc_pktsnip_t *nonce = gnrc_send_nonce_build(netif->ipv6.send_ctx.ra_nonce,
                                                      netif->ipv6.send_ctx.ra_nonce_size,
                                                      ext_opts);
        netif->ipv6.send_ctx.ra_nonce_size = 0;
        if (!nonce) {
            goto release;
        }
        ext_opts = nonce;
