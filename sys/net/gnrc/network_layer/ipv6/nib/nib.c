...
void gnrc_ipv6_nib_iface_up(gnrc_netif_t *netif)
{
    ...
    if (IS_USED(MODULE_GNRC_SEND)) {
        gnrc_ipv6_nib_send_enable(netif, GNRC_SEND_DEF_MODE);
    }
...
void gnrc_ipv6_nib_handle_timer_event(void *ctx, uint16_t type)
{
    ...
    switch (type) {
        ...
        case GNRC_IPV6_NIB_SEND_CP_SOL:
            DEBUG("nib: event GNRC_IPV6_NIB_SEND_CP_SOL, ctx=CPS context %p, time=%"PRIu32"ms\n",
                  ctx, evtimer_now_msec());
            _handle_snd_cps(ctx);
            break;
        case GNRC_IPV6_NIB_SEND_CP_ADV:
            DEBUG("nib: event GNRC_IPV6_NIB_SEND_CP_ADV, ctx=CPA context %p, time=%"PRIu32"ms\n",
                  ctx, evtimer_now_msec());
            _handle_snd_cpa(ctx);
            break;
#endif
...
static uint32_t _handle_pio(gnrc_netif_t *netif, const icmpv6_hdr_t *icmpv6,
                            ...
                            GNRC_SEND_NIB_ARG(, const gnrc_send_ip_restrict_t *ip_res));
...
#if IS_USED(MODULE_GNRC_SEND)
    if ((sec = gnrc_ipv6_nib_send_handle_rtr_sol(netif, ipv6, rtr_sol, icmpv6_len)) < 0) {
        return;
    }
    gnrc_pktsnip_t *ra_nonce = NULL;
    FOREACH_OPT(rtr_sol, opt, icmpv6_len - sizeof(ndp_rtr_sol_t)) {
        if (opt->type == NDP_OPT_NONCE) {
            size_t nonce_size;
            void *nonce = gnrc_ipv6_nib_send_handle_nonce((ndp_opt_nonce_t *)opt, &nonce_size);
            if (!(ra_nonce = gnrc_send_nonce_build(nonce, nonce_size, NULL))) {
                return;
            }
        }
    }
...
#if IS_USED(MODULE_GNRC_SEND)
            /* reply nonce in multicast is only a "MAY" */
            gnrc_pktbuf_release(ra_nonce);
#endif
                            ...
                            const ndp_rtr_adv_t *rtr_adv, size_t icmpv6_len)
{
    ...
    int sec = GNRC_SEND_STATUS_OK;
#if IS_USED(MODULE_GNRC_SEND)
    const gnrc_send_cp_t *rtr_cp = NULL;
    /* if rtr_sec is not NULL, the router can be trusted because CPS/CPA exchange */
    if ((sec = gnrc_ipv6_nib_send_handle_rtr_adv(netif, ipv6, rtr_adv, icmpv6_len, &rtr_cp)) < 0) {
        if (sec != -GNRC_SEND_STATUS_ADD_IN_PROGRESS) {
            goto ra_failure;
        }
        /* Sending of further RS is disabled.
           CP solicitation was sent and RA will be handled after certificate exchange */
        return;
    }
    gnrc_send_ip_restrict_t ip_res = GNRC_SEND_NIB_SEC_RTR_IP_INHERIT;
    if (rtr_cp) {
        gnrc_ipv6_nib_send_get_ip_restriction(&ip_res, rtr_cp);
    }
#endif
#if IS_ACTIVE(CONFIG_GNRC_IPV6_NIB_MULTIHOP_P6C)
    if (abro != NULL) {
    ...
    if (rtr_ltime != 0) {
        ...
        if (IS_USED(MODULE_GNRC_SEND)) {
            if (_nib_onl_secured(dr->next_hop) && sec != GNRC_SEND_STATUS_OK) {
                DEBUG("nib: Don´t update secured DRL entry from unsecured RA from %s\n",
                        ipv6_addr_to_str(addr_str, &ipv6->src, sizeof(addr_str)));
                goto ra_failure;
            }
            if (sec == GNRC_SEND_STATUS_OK) {
                DEBUG("nib: Setting DR secured\n");
                _nib_onl_set_secured(dr->next_hop);
            }
        }
        ...
        if (sec == GNRC_SEND_STATUS_OK || !_nib_onl_secured(dr->next_hop)) {
    ...
    FOREACH_OPT(rtr_adv, opt, icmpv6_len - sizeof(ndp_rtr_adv_t)) {
        switch (opt->type) {
            ...
            case NDP_OPT_RI:
                ...
                break;
            case NDP_OPT_PI:
                next_timeout = _min(_handle_pio(netif,
                                                gnrc_send_set_status(rtr_adv, sec),
                                                (ndp_opt_pi_t *)opt
                                                GNRC_IPV6_NIB_6LN_MULTIHOP_ARG(, abr)
                                                GNRC_SEND_NIB_ARG(, &ip_res)),
                                    next_timeout);
    ...
    /* if there is already a reachable default router */
    if ((dr = _nib_drl_get(NULL, netif->pid)) && _is_reachable(dr->next_hop)) {
        /* and it is secured */
        if (!IS_USED(MODULE_GNRC_SEND) || _nib_onl_secured(dr->next_hop)) {
            DEBUG_PUTS("nib: stop sending RS, due to probably reachable Default Router\n");
...
ra_failure:
    if (!(dr = _nib_drl_get(NULL, netif->pid)) ||
        !_is_reachable(dr->next_hop) ||
        (IS_USED(MODULE_GNRC_SEND) && !_nib_onl_secured(dr->next_hop))) {
        if (_evtimer_lookup(netif, GNRC_IPV6_NIB_SEARCH_RTR) == UINT32_MAX) {
            netif->ipv6.rs_sent = 0;
                            ...
                            const ndp_nbr_sol_t *nbr_sol, size_t icmpv6_len)
{
    ...
    int sec = GNRC_SEND_STATUS_OK;
        ...
        sec = gnrc_ipv6_nib_send_handle_nbr_sol(netif, ipv6, nbr_sol, icmpv6_len);
        FOREACH_OPT(nbr_sol, opt, icmpv6_len - sizeof(ndp_nbr_sol_t)) {
            if (opt->type == NDP_OPT_NONCE) {
                size_t nonce_size;
                void *nonce = gnrc_ipv6_nib_send_handle_nonce((ndp_opt_nonce_t *)opt, &nonce_size);
    ...
    if (tgt_netif != NULL) {
         ...
         * have. */
        if ((idx >= 0) && (dad = gnrc_netif_ipv6_addr_dad_trans(tgt_netif, idx))) {
            if (sec != GNRC_SEND_STATUS_OK) {
                if (dad > 1) {
                 ...
                 * unsecured Neighbor Advertisements and Solicitations.
                 * SEND [RFC3971] */
...
#if IS_USED(MODULE_GNRC_SEND)
        if (!pfx && ip_res && sec == GNRC_SEND_STATUS_OK) {
            sec = gnrc_ipv6_nib_send_check_prefix(&pio->prefix, pio->prefix_len, ip_res);
        }
#endif
        ...
        if (pfx) {
            if (IS_USED(MODULE_GNRC_SEND)) {
                if ((pfx->flags & _PFX_SECURED) && sec != GNRC_SEND_STATUS_OK) {
                    DEBUG("nib: Don´t update secured PLE from unsecured message\n");
                    return UINT32_MAX;
                }
                if (sec == GNRC_SEND_STATUS_OK) {
                    DEBUG("nib: Setting PLE secured\n");
                    pfx->flags |= _PFX_SECURED;
                }
            }
                           ...
                           const ndp_cp_sol_t *cp_sol, size_t icmpv6_len)
{
    ...
    FOREACH_OPT(cp_sol, opt, icmpv6_len - sizeof(ndp_cp_sol_t)) {
        if (opt > (ndp_opt_t *)(((uint8_t *)cp_sol) + icmpv6_len)) {
    ...
    gnrc_ipv6_nib_send_handle_cp_sol(netif, ipv6, cp_sol, icmpv6_len);
#endif
}
                           ...
                           const ndp_cp_adv_t *cp_adv, size_t icmpv6_len)
{
    ...
    FOREACH_OPT(cp_adv, opt, icmpv6_len - sizeof(ndp_cp_adv_t)) {
        if (opt > (ndp_opt_t *)(((uint8_t *)cp_adv) + icmpv6_len)) {
    ...
    gnrc_ipv6_nib_send_handle_cp_adv(netif, ipv6, cp_adv, icmpv6_len);
}

static void _handle_snd_cps(gnrc_send_cache_cps_t *cps_ctx)
{
    gnrc_ipv6_nib_send_handle_snd_cp_sol(cps_ctx);
}

static void _handle_snd_cpa(gnrc_send_cache_cpa_t *cpa_ctx)
{
    gnrc_ipv6_nib_send_handle_snd_cp_adv(cpa_ctx);
}
#endif
