...
#if !defined(CONFIG_AUTO_INIT_CONFIGURATION_BACKEND_PRIO) || defined(DOXYGEN)
/**
 * @brief   Priority level for auto-initialization of the default configuration backend
 */
#define CONFIG_AUTO_INIT_CONFIGURATION_BACKEND_PRIO    5000
#endif
#if !defined(CONFIG_AUTO_INIT_CONFIGURATION_IMPORT_PRIO) || defined(DOXYGEN)
/**
 * @brief   Priority level for auto-initialization of the initial configuration import
 */
#define CONFIG_AUTO_INIT_CONFIGURATION_IMPORT_PRIO      9000
#endif
...
#endif
#if !defined(CONFIG_CONFIGURATION_KEY_BUF_MAX) || defined(DOXYGEN)
/**
 * @brief   Maximum size of a configuration key buffer
 */
#define CONFIG_CONFIGURATION_KEY_BUF_MAX            256
#endif
...
/**
 * @brief   @ref CONFIG_CONFIGURATION_KEY_BUF_MAX
 */
#define CONFIGURATION_KEY_BUF_MAX           CONFIG_CONFIGURATION_KEY_BUF_MAX
...
/**
 * @brief   Handler prototype to reset the persistent storage backend
 *
 * @param[in]           be          Reference to the backend
 *
 * @return  0 on success
 */
typedef int (*conf_backend_reset_handler) (const struct conf_backend *be);
...
typedef struct conf_backend_ops {
    ...
    conf_backend_reset_handler be_reset;        /**< Backend reset function */
} conf_backend_ops_t;
...
/**
 * @brief   Get a configuration handler by its SID
 *
 * @param[in]           sid         SID of the configuration item
 *
 * @return  Pointer to the configuration handler or NULL if no handler was found for the given SID
 */
const conf_handler_t *configuration_handler(conf_sid_t sid);
...
/**
 * @brief   Reset the persistent storage backend of a configuration item by its key
 *
 * @param[in,out]       key             Identifying configuration key
 *
 * @return  0 on success
 * @return -ENOENT no handler found by key
 */
int configuration_reset(conf_key_t *key);
