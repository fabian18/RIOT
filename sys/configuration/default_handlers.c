                                        ...
                                        conf_key_buf_t *key)
{
    ...
    int err; (void)err;
    if ((err = be->ops->be_reset(be))) {
        DEBUG("configuration: backend resetting key %s failed (%d)\n",
               configuration_key_buf(key), err);
    }
                                      ...
                                      void **enc_data, size_t *enc_size)
{
    ...
    else if (!_sid_array_remainder(arr_handler, key->sid_normal) &&
             key->sid_normal < arr_handler->handler.array_id->sid_upper) {
...
#endif
    if (_encode_key_cbor(&enc, handler, sid) < 0) {
        DEBUG("configuration: failed to encode SID %"PRIu64" for CBOR text string handler %p\n",
              key->sid, (const void *)handler);
        return -ENOBUFS;
    }
                            ...
                            void **enc_data, size_t *enc_size)
{
    ...
    if (nanocbor_fmt_bool(&enc, *(const bool *)data) < 0) {
        ...
        if (IS_USED(MODULE_NANOCBOR)) {
            if (configuration_node_is_array(handler)) {
                ...
                if (configuration_node_primitive_type(handler) == CONF_PRIM_TYPE_UINT) {
                    ...
                    printf("configuration: unsupported primitive type %"PRIu32" for handler %p\n",
