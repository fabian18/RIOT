...
int configuration_reset_handler_default(const conf_handler_t *handler,
                                        conf_key_buf_t *key);
...
const conf_handler_backend_ops_t configuration_default_backend_ops = {
    ...
    .reset = configuration_reset_handler_default,
};
...
static int _configuration_handler_reset_internal(const conf_handler_t *root,
                                                 conf_key_buf_t *key)
{
    assert(root);
    assert(key);
...
#if IS_USED(MODULE_CONFIGURATION_CUSTOM_OPERATIONS)
        if (!handler->ops_be || !handler->ops_be->reset) {
            continue;
        }
        handler->ops_be->reset(handler, key);
#else
        configuration_reset_handler_default(handler, key);
#endif
    }
    _configuration_key_restore(key, &restore, _KEY_BUF(key));
    return 0;
...
{
    conf_key_buf_t key = { .sid = sid };
    const conf_handler_t *handler = configuration_get_root();
    if (_configuration_prepare_sid(&handler, &key, NULL, 0) < 0) {
        return NULL;
    }
    return handler;
}
...
{
    return _configuration_handler_reset_internal(configuration_get_root(), key);
}
