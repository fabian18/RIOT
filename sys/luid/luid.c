...
void __attribute__((weak)) luid_netdev_get(const netdev_t *netdev,
                                           ...
                                           uint8_t counter)
{
    ...
    ((uint8_t *)addr)[len - 1] ^= (counter ^ netdev->index);
    if (len > 1) {
        ((uint8_t *)addr)[len - 2] ^= (counter ^ netdev->type);
    }
#else
...
{
    luid_base(addr, sizeof(*addr));
    luid_netdev_get(netdev, addr, sizeof(*addr), 0);
    eui48_set_local(addr);
    eui48_clear_group(addr);
}
