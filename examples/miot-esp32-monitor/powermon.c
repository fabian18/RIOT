...
#include "ina3221_tables.h"
#include "powermon_params.h"
...
struct powermon {
    ...
    powermon_on_measurement_cb on_measurement;
    powermon_on_config_cb on_config;
    powermon_on_error_cb on_error;
    void* user_data;
};

static powermon_t _mons[INA3221_NUM];
static powermon_id_t _num_mons;
static event_queue_t _queue;

static uint32_t _get_update_interval_us(ina3221_enable_ch_t ech,
...
static void _update(powermon_t* mon)
{
    uint16_t ench;
    ina3221_num_samples_t ns;
    ...
    if (interval_us) {
        /* also removes old timer */
        xtimer_set(&mon->timeout, mon->update_interval_us);
    }
}

static void _measurement(powermon_t* mon)
{
    uint16_t flags = 0;
    ...
    while (polls-- && !(flags & INA3221_FLAG_CONV_READY)) {
        if ((stat = ina3221_read_flags(&mon->dev, &flags)) != INA3221_OK) {
            if (mon->on_error) {
                char msg[16];
    ...
    if (mode == INA3221_MODE_CONTINUOUS_SHUNT_ONLY ||
        ...
        if ((stat = ina3221_read_shunt_uv(&mon->dev, mon->channels,
                                          mresult.shunt_uv, NULL)) < 0) {
            if (mon->on_error) {
                char msg[16];
                snprintf(msg, sizeof(msg), "powermon: %d\n", stat);
                mon->on_error(mon, msg, mon->user_data);
    ...
    if (mode == INA3221_MODE_CONTINUOUS_BUS_ONLY ||
        ...
        mode == INA3221_MODE_CONTINUOUS_SHUNT_BUS) {
        if ((stat = ina3221_read_bus_mv(&mon->dev, mon->channels,
                                        mresult.bus_mv, NULL)) < 0) {
            if (mon->on_error) {
                char msg[16];
                snprintf(msg, sizeof(msg), "powermon: %d\n", stat);
                mon->on_error(mon, msg, mon->user_data);
...
static void _event_cb(void *arg) {
    event_post(&_queue, &(((powermon_t *)arg)->event));
}

static void* _thread(void* arg)
{
    DEBUG("Start event thread\n");
    event_queue_init(&_queue);
    for (unsigned n = 0; n < (unsigned)_num_mons; n++) {
        _update(&_mons[n]);
    }
    ...
    while ((event = event_wait(&_queue))) {
        ...
        _measurement(container_of(event, powermon_t, event));
    }
    return NULL;
}
...
int powermon_init(void)
{
    ...
    for (unsigned num = 0; num < INA3221_NUM; num++) {
        ...
        if (init == INA3221_OK) {
            ...
            _mons[success].timeout.callback = (xtimer_callback_t)_event_cb;
...
{
    return thread_create(stack, stacksize, priority,
...
powermon_t* powermon_get_mon(powermon_id_t i)
{
    if (i < _num_mons) {
        return &_mons[i];
    }
