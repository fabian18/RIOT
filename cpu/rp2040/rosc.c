...
void rosc_start(void)
{
    ...
    reg_atomic_assign(&ROSC->FREQA, 0, ROSC_FREQA_PASSWD_Msk);
    reg_atomic_assign(&ROSC->FREQB, 0, ROSC_FREQB_PASSWD_Msk);
    /* apply settings with magic value 0x9696 */
    reg_atomic_assign(&ROSC->FREQA, 0x9696U << ROSC_FREQA_PASSWD_Pos, ROSC_FREQA_PASSWD_Msk);
    reg_atomic_assign(&ROSC->FREQB, 0x9696U << ROSC_FREQA_PASSWD_Pos, ROSC_FREQB_PASSWD_Msk);
    /* default divider is 16 */
    reg_atomic_assign(&ROSC->DIV, 16U << ROSC_DIV_DIV_Pos, ROSC_DIV_DIV_Msk);
    reg_atomic_set(&ROSC->CTRL, ROSC_CTRL_ENABLE_ENABLE << ROSC_CTRL_ENABLE_Pos);
...
void rosc_stop(void)
{
    reg_atomic_set(&ROSC->CTRL, ROSC_CTRL_ENABLE_DISABLE << ROSC_CTRL_ENABLE_Pos);
}
